...
function validate_format( tag, nm, arr ){
    ...
    if ((_improves != 'smaller') && (_improves != 'larger') && (_improves != 'diff')) {
        report_status('Improves in format for '+tag+' '+nm+' needs to be smaller, larger, or diff, not '+arr[4]+'.', 'ERROR');
        return null;
    }
...
function load_this_file( data, status, xhr ){
	...
	// paint_page( true );
        //document.body.style.cursor = "wait";
         $('*').addClass('wait');
        setTimeout("paint_page( true )", 100);
        setTimeout("$('*').removeClass('wait');", 100);
    }else{
        fire_off();
    }
}
...
function user_choices_complete( location ){
    ...
    // paint_page( false );
    $('*').addClass('wait');
    setTimeout("paint_page( false )", 100);
    setTimeout("$('*').removeClass('wait');", 100);
}


// User edited their tables selection and clicked the 'Done' button, so get the settings from the popup table.
...
function calculate_all_display_values( cell, reference_cell ){
    ...
    if ((data.trim().length == 0) || (ref_data.trim().length == 0)){
        ...
        apply_diff_coloring_rule(cell, data, ref_data);
        return;
    }
    n = Number(data);
    ...
    // we know we have two numbers to work with now, but just in case we have a diff rule, try that first
    apply_diff_coloring_rule(cell, data, ref_data);
...
// Apply different background if simple difference seen, if rule for that exists
function apply_diff_coloring_rule(cell, n, n_ref){
    rule_name = cell.col_name;
    entry = config.format_map[rule_name];
    if (typeof entry == 'undefined'){return;}
    if (entry.improves != 'diff'){return;}
    /* Apply the diff coloring rule */
    if (n != n_ref){
        cell.coloring_class = 'diff_value';
    }
    return;
}
