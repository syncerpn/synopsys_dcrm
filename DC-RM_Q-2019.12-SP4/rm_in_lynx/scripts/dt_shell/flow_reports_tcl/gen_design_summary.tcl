...
proc sproc_gather_build_data { build_label build_dir } {
  ...
  ## SS
  lappend reports "report_drc"
  lappend reports "report_lvs_extract"
  lappend reports "report_lvs_compare"
  ## SS
  ...
  ## SS
  sproc_metric -cmd add -metric_name num_pv_drc_errors -display_name          "PV DRC Errors Total"
  sproc_metric -cmd add -metric_name num_pv_drc_errors_types -display_name    "PV DRC Errors Types"
  sproc_metric -cmd add -metric_name pv_drc_errors -display_name              "PV DRC Errors"
  sproc_metric -cmd add -metric_name icv_drc_run_time -display_name           "PV DRC Run Time"
  sproc_metric -cmd add -metric_name pv_lvs_extraction_result -display_name   "PV LVS extraction Result"
  sproc_metric -cmd add -metric_name pv_lvs_extraction_errors -display_name   "PV LVS extraction Errors"
  sproc_metric -cmd add -metric_name pv_lvs_comparison_result -display_name   "PV LVS comparison Result"
  sproc_metric -cmd add -metric_name icv_lvs_run_time -display_name           "PV LVS Run Time"
  ## SS
  ...
  foreach task_info $task_info_list {
    ...
    ## SS
    unset -nocomplain drc
    unset -nocomplain lvs_extract
    unset -nocomplain lvs_compare
    ## SS
    ...
    foreach file $task_report_list {
      ...
      ## SS
      if { [file extension $file] == ".report_drc" } {
        array set drc [sproc_parse_report_drc -file $file]
        set pv_drc_file $file
      }

      if { [file extension $file] == ".report_lvs_extract" } {
        array set lvs_extract [sproc_parse_report_lvs_extract -file $file]
        set lvs_extract_file $file
      }

      if { [file extension $file] == ".report_lvs_compare" } {
        array set lvs_compare [sproc_parse_report_lvs_compare -file $file]
        set lvs_compare_file $file
      }
      ## SS
    ...
    if { [info exists drc] } {
      ...
      if { [info exists drc(num_pv_drc_errors)] } {
        set data $drc(num_pv_drc_errors)
        set link $pv_drc_file:$drc(num_pv_drc_errors,line_number)
        set misc_status($build_label,$task_id,num_pv_drc_errors,value) $data
        set misc_status($build_label,$task_id,num_pv_drc_errors,link) $link
      }

      if { [info exists drc(num_pv_drc_errors_types)] } {
        set data $drc(num_pv_drc_errors_types)
        set link $pv_drc_file:$drc(num_pv_drc_errors_types,line_number)
        set misc_status($build_label,$task_id,num_pv_drc_errors_types,value) $data
        set misc_status($build_label,$task_id,num_pv_drc_errors_types,link) $link
      }

      if { [info exists drc(pv_drc_errors)] } {
        set data $drc(pv_drc_errors)
        set link $pv_drc_file:$drc(pv_drc_errors,line_number)
        set misc_status($build_label,$task_id,pv_drc_errors,value) $data
        set misc_status($build_label,$task_id,pv_drc_errors,link) $link
      }

      if { [info exists drc(icv_drc_run_time)] } {
        set data $drc(icv_drc_run_time)
        set link $pv_drc_file:$drc(icv_drc_run_time,line_number)
        set misc_status($build_label,$task_id,icv_drc_run_time,value) $data
        set misc_status($build_label,$task_id,icv_drc_run_time,link) $link
      }

    }

    if { [info exists lvs_extract] } {

      if { [info exists lvs_extract(pv_lvs_extraction_result)] } {
        set data $lvs_extract(pv_lvs_extraction_result)
        set link $lvs_extract_file:$lvs_extract(pv_lvs_extraction_result,line_number)
        set misc_status($build_label,$task_id,pv_lvs_extraction_result,value) $data
        set misc_status($build_label,$task_id,pv_lvs_extraction_result,link) $link
      }

      if { [info exists lvs_extract(pv_lvs_extraction_errors)] } {
        set data $lvs_extract(pv_lvs_extraction_errors)
        set link $lvs_extract_file:$lvs_extract(pv_lvs_extraction_errors,line_number)
        set misc_status($build_label,$task_id,pv_lvs_extraction_errors,value) $data
        set misc_status($build_label,$task_id,pv_lvs_extraction_errors,link) $link
      }

    }

    if { [info exists lvs_compare] } {

      if { [info exists lvs_compare(pv_lvs_comparison_result)] } {
        set data $lvs_compare(pv_lvs_comparison_result)
        set link $lvs_compare_file:$lvs_compare(pv_lvs_comparison_result,line_number)
        set misc_status($build_label,$task_id,pv_lvs_comparison_result,value) $data
        set misc_status($build_label,$task_id,pv_lvs_comparison_result,link) $link
      }

      if { [info exists lvs_compare(icv_lvs_run_time)] } {
        set data $lvs_compare(icv_lvs_run_time)
        set link $lvs_compare_file:$lvs_compare(icv_lvs_run_time,line_number)
        set misc_status($build_label,$task_id,icv_lvs_run_time,value) $data
        set misc_status($build_label,$task_id,icv_lvs_run_time,link) $link
      }
    }
...
proc attrColor { metric_name metric_value } {
  ...
  switch $metric_name {
    pv_lvs_extraction_result -
    pv_lvs_comparison_result {
      if { $metric_value == "FAIL" } {
        set attr "#ffffff:#ff0000"
      } elseif { $metric_value == "PASS" } {
        set attr "#ffffff:#228b22"
      }
    }
    pv_drc_errors -
    pv_lvs_extraction_errors {
        if { $metric_value != "-1"  && $metric_value != "" } {
          set attr "#ffffff:#0000ff"
      }
    }
  }
